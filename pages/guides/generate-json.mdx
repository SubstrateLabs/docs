...
<Callout emoji="֍">Learn how to generate structured JSON output using ComputeJSON</Callout>

Generating JSON with LLMs is a useful technique that is broadly applicable to many tasks – from data extraction, to data wrangling, to function calling. You'll find that it's also very useful in the context of chained workflows.

In this guide, we'll show you how to generate JSON using <ComputeJSON/>. Substrate uses robust structured generation techniques at inference time – which means valid JSON output is guaranteed, and fast.
